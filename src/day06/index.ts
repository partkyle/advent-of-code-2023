...
class Input {
  ...
  constructor(rawInput: string) {
    ...
    this.times = timesLine.split(/:\s+/)[1].split(/\s+/).map(s => parseInt(s))
    this.distances = distancesLine.split(/:\s+/)[1].split(/\s+/).map(s => parseInt(s))

    this.timePart2 = parseInt(timesLine.split(/:\s+/)[1].replace(/\s/g, ''))
    this.distancePart2 = parseInt(distancesLine.split(/:\s+/)[1].replace(/\s/g, ''))
  }
}
...
const part1 = (rawInput: string) => {
  ...
  return _.zip(input.times, input.distances).map( v => {
    let [time, distance] = v
    return _.range(0, time).map(t => calcDistance(t, time)).filter(t => t > distance).length
